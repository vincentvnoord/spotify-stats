...
export const TimeFrameButton = ({ children, onClick, timeFrame }: { children: React.ReactNode, onClick?: () => void, timeFrame: string }) => {
    ...
    return (
        <Link href={currentPath}>
            <button onClick={onClick} className={`transition-colors text-sm sm:text-base duration-200 text-nowrap ease-in-out sm:min-w-20 md:min-w-28 font-semibold p-3 md:p-3 rounded-full ${selected ? selectedStyle : defaultStyle}`}>
                {children}
            </button>
        </Link>
    )
